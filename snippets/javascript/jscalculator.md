...
// accept the second number from the user and check if it is a number and a non-zero for division
while(isNaN(number2) || (number2===0 && operator==='/')) {  
  if(number2===0 && operator==='/'){
   number2 = parseFloat(prompt ('Please enter non-zero number to divide with : '));
  }
 else 
...
switch(operator){
  ...
  case '*' :  // use * (multiplication) operator to multiply two numbers  
      ...
      break;
  default : // The is the default operation i.e. division
